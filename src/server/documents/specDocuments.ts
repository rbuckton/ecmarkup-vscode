...
const trace = new TraceSwitch("ecmarkup-vscode-specdocuments", /*enabled*/ false);
...
export class SpecDocuments extends EventEmitter {
    ...
    public set(uri: string, textDocument: TextDocument) {
        if (this._documents.has(uri)) {
            const document = this._documents.get(uri);
            if (!document.ready) document.abort();
        }

        return this.addDocument(new SpecDocument(this._documentManager, uri, textDocument));
    }
    ...
    private addDocument(document: SpecDocument) {
        trace.log(`addDocument(${document.uri})`);
